...
\begin{abstract}
Cholesterol is a central building block of biomembranes, where it induces orientational order, slows down diffusion, and renders membranes generally stiffer. Molecular dynamics simulations have played a crucial role in resolving these effects and the underlying molecular picture, yet it has recently become evident that different simulation models provide quantitatively different behavior. As the field progresses towards simulations of more complex membranes, the fundamental interactions such as those between cholesterol and phospholipids---the key building blocks of biomembranes---such be modelled accurately.
%
Here, we perform a systematic comparison of the commonly used force fields to describe the structure and dynamics of binary mixtures of phosphatidylcholine and cholesterol. Comparison to deuterium order parameters and lateral diffusion coefficients from NMR as well as electron density profiles from X-ray scattering reveals that none of the existing force field outperforms the others across the physiologically relevant cholesterol concentrations. The Amber-compatible force fields show the smallest deviations from experiment with Slipids and Lipid17 models excelling at low and high cholesterol concentrations, respectively. The development of force fields or the refinement of existing ones can benefit from the included X-ray scattering data set.
\end{abstract}
...
\todo{Scattering methods incomplete}
...
\todo{Results: Add missing 30\% data to the scattering plots}\\
