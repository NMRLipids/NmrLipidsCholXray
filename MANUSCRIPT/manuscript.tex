...
with 13C or 2H NMR techniques, give indirect information about structural
...
experiments can be explained by increased trans conformations in acyl chains \cite{??},
which is suggested to play critical role in phase behaviour in PC lipid-cholesterol
mixtures \cite{??}. Consequently, the correct cholesterol ordering effect is
...
are shown in Fig. \ref{OrderParametersCHOL}. Phase separation is not observed for this system \cite{ferreira13,??},
...
and have been widely used to validate lipid bilayer simulations \cite{??}. 
While sophisticated models are applied to extract area per molecule and thicknes for single
component lipid bilayers, the results for multicomponent systems are more difficult to interpret \cite{??}.
In such case the validation can be done by comparing directly measurable form factors between
simulations and experiments \cite{??}. If simulation reproduces the form factor, it can be also
